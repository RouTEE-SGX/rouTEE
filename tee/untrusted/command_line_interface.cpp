...
static void validate_address(std::string my_address) {
    if (my_address.length() != BITCOIN_ADDRESS_LEN) {
        printf("Given bitcoin address: %s", my_address.c_str());
        error("Bitcoin address length incorrect");
    }
}
...
// issues a command to the local routee enclave for a specific address
static void local_issue_command_for_address_routee(int sockfd, unsigned int command, std::string my_address) {
    struct LocalGenericRouteeMsg msg;

    msg.msg_op[0] = command;
    memcpy((char *) msg.my_address, my_address.c_str(), BITCOIN_ADDRESS_LEN);

    send_on_socket((char*) &msg, sizeof(struct LocalGenericRouteeMsg), sockfd); 
}
...
static void local_primary_routee(int argc, char* argv[]) {
    struct LocalAssignmentMsg msg;
    msg.msg_op[0] = OP_LOCAL_PRIMARY_ROUTEE;
    msg.use_monotonic_counters = use_monotonic_counters;
    send_on_socket((char*) &msg, sizeof(struct LocalAssignmentMsg), local_sockfd);
    local_wait_for_response_and_close(local_sockfd, "local_primary_routee");
}
...
static void local_set_routing_fee_routee(int argc, char* argv[]) {
    if (!enough_arguments_for_command(1, argc)) usage();

    unsigned long long routing_fee_to_pay = strtoull(argv[optind + 1], NULL, 10); // routing fee for using rouTEE

    struct LocalSetRoutingFeeRouteeMsg msg;
    msg.msg_op[0] = OP_LOCAL_SET_ROUTING_FEE_ROUTEE;
    msg.routing_fee = routing_fee_to_pay;

    // send setup message
    send_on_socket((char*) &msg, sizeof(struct LocalSetRoutingFeeRouteeMsg), local_sockfd);
    local_wait_for_response_and_close(local_sockfd, "local_set_routing_fee_routee");
}

static void local_set_routing_fee_address_routee(int argc, char* argv[]) {
    if (!enough_arguments_for_command(1, argc)) usage();

    std::string routing_fee_address(argv[optind + 1]);

    struct LocalSetRoutingFeeAddressRouteeMsg msg;
    msg.msg_op[0] = OP_LOCAL_SET_ROUTING_FEE_ADDRESS_ROUTEE;
    memcpy(msg.routing_fee_address, routing_fee_address.c_str(), BITCOIN_ADDRESS_LEN);

    // send setup message
    send_on_socket((char*) &msg, sizeof(struct LocalSetRoutingFeeAddressRouteeMsg), local_sockfd);
    local_wait_for_response_and_close(local_sockfd, "local_set_routing_fee_address_routee");
}

// create channel
static void local_create_channel_routee(int argc, char* argv[]) {
    if (!enough_arguments_for_command(3, argc)) usage();
    
    std::string my_address(argv[optind + 1]); // my bitcoin address in B58 format to return funds to
    // extract deposits one at a time
    std::string txid(argv[optind + 2]);
    unsigned long long deposit_amount = strtoull(argv[optind + 3], NULL, 10);

    // construct deposits made message with data
    struct LocalCreateChannelRouteeMsg msg = {}; // zero initialize the struct
    msg.msg_op[0] = OP_LOCAL_CREATE_CHANNEL_ROUTEE;
    memcpy(msg.my_address, my_address.c_str(), BITCOIN_ADDRESS_LEN);
    memcpy(msg.txid, txid.c_str(), BITCOIN_TX_HASH_LEN);
    ...
    // send setup message
    send_on_socket((char*) &msg, sizeof(struct LocalCreateChannelRouteeMsg), local_sockfd);
    local_wait_for_response_and_close(local_sockfd, "local_create_channel_routee");
}

// print state
static void local_print_state_routee(int argc, char* argv[]) {
    if (!enough_arguments_for_command(0, argc)) usage();
    
    local_issue_command(local_sockfd, OP_LOCAL_PRINT_STATE_ROUTEE);
    local_wait_for_response_and_close(local_sockfd, "local_print_state_routee");
}

static void local_settle_balance_routee(int argc, char* argv[]) {
    if (!enough_arguments_for_command(1, argc)) usage();
    ...
    // construct deposits made message with data
    struct LocalSettleBalanceRouteeMsg msg; // zero initialize the struct
    msg.msg_op[0] = OP_LOCAL_SETTLE_BALANCE_ROUTEE;
    memcpy(msg.my_address, my_address.c_str(), BITCOIN_ADDRESS_LEN);

    // send setup message
    send_on_socket((char*) &msg, sizeof(struct LocalSettleBalanceRouteeMsg), local_sockfd);
    local_wait_for_response_and_close(local_sockfd, "local_settle_balance_routee");
}

/*
static void local_make_settle_transaction_routee(int argc, char* argv[]) {
    if (!enough_arguments_for_command(1, argc)) usage();
...
static void local_do_multihop_payment_routee(int argc, char* argv[]) {
    if (!enough_arguments_for_command(4, argc)) usage();

    std::string sender_address(argv[optind + 1]); // my bitcoin address in B58 format to return funds to
    std::string receiver_address(argv[optind + 2]);
    unsigned long long amount = strtoull(argv[optind + 3], NULL, 10);
    unsigned long long routing_fee_to_pay = strtoull(argv[optind + 4], NULL, 10); 

    // construct deposits made message with data
    struct LocalDoMultihopPaymentRouteeMsg msg = {}; // zero initialize the struct
    msg.msg_op[0] = OP_LOCAL_DO_MULTIHOP_PAYMENT_ROUTEE;
    memcpy(msg.sender_address, sender_address.c_str(), BITCOIN_ADDRESS_LEN);
    memcpy(msg.receiver_address, receiver_address.c_str(), BITCOIN_ADDRESS_LEN);
    msg.amount = amount;
    msg.routing_fee = routing_fee_to_pay;

    // send setup message
    send_on_socket((char*) &msg, sizeof(struct LocalDoMultihopPaymentRouteeMsg), local_sockfd);
    local_wait_for_response_and_close(local_sockfd, "local_do_multihop_payment_routee");
}
...
static void local_make_owner_key_routee(int argc, char* argv[]) {
    ...
    local_issue_command(local_sockfd, OP_LOCAL_MAKE_OWNER_KEY_ROUTEE);
    local_wait_for_response_and_close(local_sockfd, "local_make_owner_key_routee");
}

static void local_load_owner_key_routee(int argc, char* argv[]) {
    if (!enough_arguments_for_command(0, argc)) usage();

    local_issue_command(local_sockfd, OP_LOCAL_LOAD_OWNER_KEY_ROUTEE);
    local_wait_for_response_and_close(local_sockfd, "local_load_owner_key_routee");
}
...
static void validate_and_execute_command(int argc, char* argv[]) {
    ...
    } else if (streq(command, "routee_primary")) {
        printf("primary executed\n");
        local_primary_routee(argc, argv);

    } else if (streq(command, "routee_set_routing_fee")) {
        printf("set routing fee executed\n");
        local_set_routing_fee_routee(argc, argv);

    } else if (streq(command, "routee_set_fee_address")) {
        printf("set routing fee address executed\n");
        local_set_routing_fee_address_routee(argc, argv);

    } else if (streq(command, "routee_create_channel")) {
        printf("create channel executed\n");
        local_create_channel_routee(argc, argv);

    } else if (streq(command, "routee_print_state")) {
        printf("print state executed\n");
        local_print_state_routee(argc, argv);

    } else if (streq(command, "routee_settle_balance")) {
        printf("settle balance executed\n");
        local_settle_balance_routee(argc, argv);
/*
    } else if (streq(command, "make_settle_transaction_routee")) {
        printf("make settle transaction executed\n");
        local_make_settle_transaction_routee(argc, argv);
*/
    } else if (streq(command, "routee_do_multihop_payment")) {
        printf("do multihop payment executed\n");
        local_do_multihop_payment_routee(argc, argv);
