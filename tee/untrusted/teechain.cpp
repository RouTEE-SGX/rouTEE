...
static void primary_routee(char *data, int client_fd) {
    struct LocalAssignmentMsg *msg = (struct LocalAssignmentMsg*) data;
    //log_debug("primary()");

    char user_output[MAX_ECALL_RETURN_LENGTH];
    int ecall_return;
    int command_return = ecall_primary_routee(global_eid, &ecall_return, msg->use_monotonic_counters, user_output);
    if (command_return != SGX_SUCCESS || ecall_return != 0) {
        send_ack(client_fd, OP_LOCAL_FAIL, "ecall_primary_routee");
        error("ecall_primary_routee");
    }
...
static void set_routing_fee_routee(char* data, int client_fd) {
    struct LocalSetRoutingFeeRouteeMsg msg = *((struct LocalSetRoutingFeeRouteeMsg*) (data));
    unsigned long long routing_fee = msg.routing_fee; 

    char user_output[MAX_ECALL_RETURN_LENGTH];
    //printf("FEEFEE: %s\n", user_output);
    int ecall_return;
    int command_return = ecall_set_routing_fee_routee(global_eid, &ecall_return, routing_fee, user_output);
    if (command_return != SGX_SUCCESS || ecall_return != 0) {
        error("ecall_set_routing_fee_routee");
    }
    //printf("FEEFEEFEE: %s\n", user_output);
    send_ack(client_fd, OP_LOCAL_ACK, user_output);
}

static void set_routing_fee_address_routee(char* data, int client_fd) {
    struct LocalSetRoutingFeeAddressRouteeMsg *msg = (struct LocalSetRoutingFeeAddressRouteeMsg*) (data);
    std::string routing_fee_address(msg->routing_fee_address, BITCOIN_ADDRESS_LEN); 

    char user_output[MAX_ECALL_RETURN_LENGTH];
    int ecall_return;
    int command_return = ecall_set_routing_fee_address_routee(global_eid, &ecall_return, routing_fee_address.c_str(), routing_fee_address.length(), user_output);
    if (command_return != SGX_SUCCESS || ecall_return != 0) {
        error("ecall_set_routing_fee_address_routee");
    }

    send_ack(client_fd, OP_LOCAL_ACK, user_output);
}

static void create_channel_routee(char* data, int client_fd) {
    struct LocalCreateChannelRouteeMsg *msg = (struct LocalCreateChannelRouteeMsg*) (data);
    std::string my_address(msg->my_address, BITCOIN_ADDRESS_LEN);
    std::string txid(msg->txid, BITCOIN_TX_HASH_LEN);
    unsigned long long deposit_amount = msg->deposit_amount;
    unsigned long long remote_host_len = msg->remote_host_len;
    std::string remote_host(msg->remote_host, remote_host_len); 
    unsigned long remote_port = msg->remote_port;

    char user_output[MAX_ECALL_RETURN_LENGTH];
    int ecall_return;
    int command_return = ecall_create_channel_routee(global_eid, &ecall_return, my_address.c_str(), my_address.length(), txid.c_str(), txid.length(), deposit_amount, user_output);
    if (command_return != SGX_SUCCESS || ecall_return != 0) {
        error("ecall_create_channel_routee");
    }

    send_ack(client_fd, OP_LOCAL_ACK, user_output);
}

static void print_state_routee(char* data, int client_fd) {
    char user_output[MAX_ECALL_RETURN_LENGTH];
    int ecall_return;
    int command_return = ecall_print_state_routee(global_eid, &ecall_return, user_output);
    if (command_return != SGX_SUCCESS || ecall_return == REQUEST_CRASHED) {
        send_ack(client_fd, OP_LOCAL_FAIL, "ecall_print_state_routee");
        error("ecall_print_state_routee");
    }

    if (ecall_return == REQUEST_FAILED) {
        printf("Unable to print routee state!");
        send_ack(client_fd, OP_LOCAL_ACK, "Unable to print routee state");
        return;
    }
...
static void settle_balance_routee(char* data, int client_fd) {
    struct LocalSettleBalanceRouteeMsg *msg = (struct LocalSettleBalanceRouteeMsg*) data;
    std::string my_address(msg->my_address, BITCOIN_ADDRESS_LEN);
    //log_debug("settle(%s)", my_address.c_str());

    char user_output[MAX_ECALL_RETURN_LENGTH];
    int ecall_return;
    int command_return = ecall_settle_balance_routee(global_eid, &ecall_return, my_address.c_str(), my_address.length(), user_output);
    if (command_return != SGX_SUCCESS || ecall_return == REQUEST_CRASHED) {
        send_ack(client_fd, OP_LOCAL_FAIL, "ecall_settle_balance_routee");
        error("ecall_settle_balance_routee");
    }

    if (ecall_return == REQUEST_FAILED) {
        printf("Unable to settle routee balance!");
        send_ack(client_fd, OP_LOCAL_ACK, "Unable to settle routee balance");
        return;
    }
...
static void do_multihop_payment_routee(char* data, int client_fd) {
    struct LocalDoMultihopPaymentRouteeMsg *msg = (struct LocalDoMultihopPaymentRouteeMsg*) (data);
    ...
    char user_output[MAX_ECALL_RETURN_LENGTH];
    int ecall_return;
    int command_return = ecall_do_multihop_payment_routee(global_eid, &ecall_return, sender_address.c_str(), sender_address.length(), receiver_address.c_str(), receiver_address.length(), amount, user_output);
    if (command_return != SGX_SUCCESS || ecall_return != 0) {
        error("ecall_do_multihop_payment_routee");
    }
...
static void setup_deposit_request_routee(char* data, int client_fd) {
    ...
    if (ecall_return == REQUEST_FAILED) {
        printf("Unable to setup deposit request routee!");
        send_ack(client_fd, OP_LOCAL_ACK, "Unable to setup deposit request routee");
        return;
    }
...
static void make_owner_key_routee(char* data, int client_fd) {
    char user_output[MAX_ECALL_RETURN_LENGTH];
    int ecall_return;
    int command_return = ecall_make_owner_key_routee(global_eid, &ecall_return, user_output);
    if (command_return != SGX_SUCCESS || ecall_return == REQUEST_CRASHED) {
        send_ack(client_fd, OP_LOCAL_FAIL, "ecall_make_owner_key_routee");
        error("ecall_make_owner_key_routee");
    }

    if (ecall_return == REQUEST_FAILED) {
        printf("Unable to make owner key routee!");
        send_ack(client_fd, OP_LOCAL_ACK, "Unable to make owner key routee");
        return;
    }
...
static void load_owner_key_routee(char* data, int client_fd) {
    char user_output[MAX_ECALL_RETURN_LENGTH];
    int ecall_return;
    int command_return = ecall_load_owner_key_routee(global_eid, &ecall_return, user_output);
    if (command_return != SGX_SUCCESS || ecall_return == REQUEST_CRASHED) {
        send_ack(client_fd, OP_LOCAL_FAIL, "ecall_load_owner_key_routee");
        error("ecall_load_owner_key_routee");
    }

    if (ecall_return == REQUEST_FAILED) {
        printf("Unable to load owner key routee!");
        send_ack(client_fd, OP_LOCAL_ACK, "Unable to load owner key routee");
        return;
    }
...
static void process_packet(char* packet, int pktlen, int client_fd) {
    ...
    } else if (islocalhost && (operation == OP_LOCAL_PRIMARY_ROUTEE)) { // received local primary
        primary_routee(&packet[MSG_LEN_BYTES], client_fd);

    } else if (islocalhost && (operation == OP_LOCAL_SET_ROUTING_FEE_ROUTEE)) {
        set_routing_fee_routee(&packet[MSG_LEN_BYTES], client_fd);

    } else if (islocalhost && (operation == OP_LOCAL_SET_ROUTING_FEE_ADDRESS_ROUTEE)) {
        set_routing_fee_address_routee(&packet[MSG_LEN_BYTES], client_fd);

    } else if (islocalhost && (operation == OP_LOCAL_CREATE_CHANNEL_ROUTEE)) {
        create_channel_routee(&packet[MSG_LEN_BYTES], client_fd);
        
    } else if (islocalhost && (operation == OP_LOCAL_PRINT_STATE_ROUTEE)) {
        print_state_routee(&packet[MSG_LEN_BYTES], client_fd);

    } else if (islocalhost && (operation == OP_LOCAL_SETTLE_BALANCE_ROUTEE)) {
        settle_balance_routee(&packet[MSG_LEN_BYTES], client_fd);

    } else if (islocalhost && (operation == OP_LOCAL_DO_MULTIHOP_PAYMENT_ROUTEE)) {
        do_multihop_payment_routee(&packet[MSG_LEN_BYTES], client_fd);

    } else if (islocalhost && (operation == OP_LOCAL_SETUP_DEPOSIT_REQUEST_ROUTEE)) {
        setup_deposit_request_routee(&packet[MSG_LEN_BYTES], client_fd);

    } else if (islocalhost && (operation == OP_LOCAL_MAKE_OWNER_KEY_ROUTEE)) {
        make_owner_key_routee(&packet[MSG_LEN_BYTES], client_fd);

    } else if (islocalhost && (operation == OP_LOCAL_LOAD_OWNER_KEY_ROUTEE)) {
        load_owner_key_routee(&packet[MSG_LEN_BYTES], client_fd);

    } else if (islocalhost && (operation == OP_LOCAL_INSERT_BLOCK_ROUTEE)) {
        insert_block_routee(&packet[MSG_LEN_BYTES], client_fd);
