...
{
    if (!IsHex(strHexBlkHeader))
        return false;

    std::vector<unsigned char> blockHeaderData(ParseHex(strHexBlkHeader));
    CDataStream ssBlockHeader(blockHeaderData, SER_NETWORK, PROTOCOL_VERSION);
    try {
