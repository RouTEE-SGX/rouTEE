...
void make_owner_key_routee() {
    // For owner certification
    sgx_status_t ret;
	//unsigned char alpha[32];
    unsigned long long alpha;
	ret = sgx_read_rand((unsigned char *) &alpha, 8);
    if(ret != SGX_SUCCESS)
	{
	    printf("sgx_read_rand failed %#x", ret);
	    abort();
	}
    state.owner_certificate = alpha;
...
int ecall_set_routing_fee_routee(unsigned long long routing_fee, char* user_output) {
    ...
    state.routing_fee = routing_fee;
    //std::string output = "Set routing fee for rouTEE: " + std::to_string(state.routing_fee) + "\n";
    //memcpy(user_output, output.c_str(), output.length() + 1);
...
static std::string generate_settle_transaction_for_routee(Account* user_account) {
    /*
    ...
    std::string generated_transaction = create_settle_transaction_for_routee(user_account, miner_fee);

    return sign_settle_transaction_for_routee(user_account, generated_transaction);
    */
    return "generate_settle_transaction_for_routee";
}

static bool settle_routee(Account* user_account, std::string& output) {
    ...
    */
    ...
    generated_settle_transaction = generate_settle_transaction_for_routee(user_account);
    // TODO: send notification of settlement to remote host, so they know too.

    //output += "This transaction has settled your routee: " + /*user_account + */", please broadcast it to the Bitcoin network!\n";
...
int ecall_settle_balance_routee(const char* my_address, int address_len, char* user_output) {
    ...
    std::string addr(my_address, address_len); 
    Account* user_account = state.users.find(addr)->second;
    
    if (user_account->settle_request == true) {
        printf("Cannot settle routee balance; already requested for settle!");
        return REQUEST_FAILED;
    }

    user_account->settle_request = true;
    user_account->settle_amount = user_account->balance;
    //settle_transaction = 


    std::string output;
    if (!settle_routee(user_account, output)) {
...
int ecall_do_multihop_payment_routee(const char* sender_address, int sender_address_len, const char* receiver_address, int receiver_address_len, unsigned long long amount, char* user_output) {
    ...
    if (!account_exist(sender_address, sender_address_len)) {
	    printf("Cannot do multihop payment routee; there are no sender account!");
        return REQUEST_FAILED;
    }

    Account* sender_account = state.users.find(std::string(sender_address, sender_address_len))->second;
    // ensure that we have the funds
    if (amount <= 0 || amount + state.routing_fee > sender_account->balance) {
        printf("Cannot send amount %llu, balance is %llu, routing fee is %llu", amount, sender_account->balance, state.routing_fee);
        return REQUEST_FAILED; // don't error, just return
    }

    if (!account_exist(receiver_address, receiver_address_len)) {
        create_new_account(receiver_address, receiver_address_len);
    }

    Account* receiver_account = state.users.find(std::string(receiver_address, receiver_address_len))->second;

    sender_account->balance -= (amount + state.routing_fee);
    receiver_account->balance += amount;

    sender_account->pending_fee = state.routing_fee / 2;
    receiver_account->pending_fee = state.routing_fee - state.routing_fee / 2;

    sender_account->nonce++;

    state.stateID++;

    std::string output = "Payment resolved: " + std::string(sender_address, sender_address_len) + " -> " + std::string(receiver_address, receiver_address_len) + "\n";
...
int ecall_load_owner_key_routee(char* user_output) {
    if (!check_state(Ready)) {
	    printf("Cannot load owner key routee; this enclave is not a primary!");
        return 1;
    }
    return 0;
}
